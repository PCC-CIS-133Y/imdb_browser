...
class ShowBrowserApp:
    ...
    def setup_genres(self):
        self.__genre_combo['values'] = [ShowGenre.ALL_GENRES] + [x.get_genre() for x in ShowService.fetch_genres()]
        self.__genre_combo.current(0)

    # Setup the types combobox so that the user can select which types of shows to browse.
    def setup_types(self):
        self.__type_combo['values'] = [ShowType.ALL_TYPES] + [x.get_type() for x in ShowService.fetch_types()]
    ...
    def fetch_data(self):
        ...
        data = ShowService.fetch_popular_shows(genre, show_type, min_votes)
